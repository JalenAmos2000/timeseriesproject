...
libraries('fpp3','lubridate')

#==== Fpp2 version of Reading the Data
Appl <- read.csv("C:/Users/jalen/Downloads/EXST_7087_Project/Data/Appl2.csv",header = TRUE, sep = ",")

df=Appl%>%filter(Date>=as.Date("2020-07-24"))
Df1 <-ts(df$Close, frequency = 365.25, start=c(2020,07,24))
autoplot(Df1)
p1=ggplot()+
    geom_line(data = Appl, aes(x = Date, y = Close)) + 
    ylab('Closing Price')+
    theme_bw()
p1
...
recent_closingprice <- Appl |>
    filter(Daily >= as.Date("2019-01-01"))
...
# we spilting the data up (Remainder is what is leftover) Additive decomposition
